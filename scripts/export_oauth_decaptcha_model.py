...
def to_model_payload(checkpoint):
    ...
    digits = checkpoint.get("digits", 4) if isinstance(checkpoint, dict) else 4
    return {
        "digits": digits,
        "eps": 1e-5,
        "featureHeight": 5,
...
def render_model_script(payload):
  ...
  namespace.oauthDecaptchaModel = model;
})();
""" % payload_json
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export the OAuth decaptcha checkpoint into a bundled TS model file."
    )
    parser.add_argument(
        "--checkpoint",
        default=str(project_root.parent / "ccxpDecaptcha" / "out" / "oauth" / "best.pt"),
        help="Path to the source checkpoint.",
    )
    parser.add_argument(
        "--output",
        default=str(project_root / "src" / "content.oauth.decaptcha.model.ts"),
